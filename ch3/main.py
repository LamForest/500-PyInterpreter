...
class Frame:
    ...
    def _impl_LOAD_NAME(self, instruction : Instruction):
        """
        cpython代码逻辑，来源: https://tenthousandmeters.com/blog/python-behind-the-scenes-5-how-variables-are-implemented-in-cpython/
        If the name is not in f_locals, the VM looks up the value in the dictionary of global variables f_globals.
        And if the name is not in f_globals either, the VM looks up the value in f_builtins.
        The f_builtins field of a frame object points to the dictionary of the builtins module,
        which contains built-in types, functions, exceptions and constants. 
        
        If the name is not there, the VM gives up and sets the NameError exception. 
        """
        index = instruction.arg
        ...
        if name in self.locals:
            ...
            raise NameError("LOAD NAME: {} not found in local, global, builtin".format(name))
...
class TestAdd(unittest.TestCase):
    ...
    def test_call_recursive(self):
        ...
        frame = Frame(cobj)
        frame.exec()
        self.assertEqual(frame.locals["f0"], 1)
        self.assertEqual(frame.locals["f1"], 1)
        self.assertEqual(frame.locals["f2"], 2)
        self.assertEqual(frame.locals["f5"], 5)
        self.assertEqual(frame.locals["f9"], 55)
